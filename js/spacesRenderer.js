...
var spacesRenderer = {
    ...
    renderSpaces: function(spaces) {
        ...
        if(selectedSpace){
            if(selectedSpace.sessionId !==false){
                this.selectSpace(self.getSeletedSpaceEl(selectedSpace), false);    
            }else{ 
                this.selectSpace(self.getFirstSpaceEl(), false);       
            }           
        }else{
            this.selectSpace(self.getFirstSpaceEl(), false);
        }
    ...
    renderSpaceEl: function(space) {
        var self = this,
            ...
            listEditable = document.createElement('span');
        ...
        listEditable.className = 'spaceEdit';

        listTitle.innerHTML = name;
        listDetail.innerHTML = this.getTabDetailsString(space);
        listEditable.innerHTML = '<i class="fa fa-edit" aria-hidden="true"></i>';

        listContainer.appendChild(listTitle);
        listContainer.appendChild(listDetail);
        listContainer.appendChild(listEditable);
    ...
    getSeletedSpaceEl: function(selectedSpace) {
        var allSpaceEls = document.querySelectorAll('#spacesList .space'),
            selectedSpaceEl = false;
        Array.prototype.some.call(allSpaceEls, function (spaceEl) {
            if(selectedSpace.sessionId == spaceEl.getAttribute("data-sessionId")){
                selectedSpaceEl = spaceEl;
                return true;
            }           
        });
        return selectedSpaceEl;
    },
    ...
    updateSpacesList: function() {
        ...
        //highlight the selected space el in the visible list        
       /* if(selectedSpace){
            this.selectSpace(self.getSeletedSpaceEl(selectedSpace), false);    
        }else{
            this.selectSpace(self.getFirstSpaceEl(), false);
