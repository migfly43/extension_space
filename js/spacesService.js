...
(function (window) {
    ...
    var spacesService = {
        ...
        handleTabRemoved: function(tabId, removeInfo, callback) {
            ...
            // Get Tab info of Removed and remove pinned tab from other session
            var sessionTabRemoved = this.getSessionByWindowId(removeInfo.windowId);
            var removedTab;
            if(sessionTabRemoved){
                sessionTabRemoved.tabs.forEach(function(tab){
                if(tab.id == tabId){
                        removedTab = tab;
                    }
                });
                if(removedTab.pinned == true){
                    this. handlePinnedTabUpdated(removedTab); 
                }   
            }            
            
            //NTE: isWindowClosing is true if the window cross was clicked causing the tab to be removed.
        ...
        handleTabMoved: function(tabId, moveInfo, callback) {           
            ...
            //this.handlePinnedTabMoved(moveInfo.windowId);
        },
        handleTabUpdated: function(tab, changeInfo, callback) {
            ...
            if(changeInfo.pinned === false){
                this.handlePinnedTabUpdated(tab);    
            }
        },
        //when move pinned tab, repalce order of pinned tab as order of current session in another seesions
        handlePinnedTabMoved: function(movedPinnedWindowId){
            var currentSession = this.getSessionByWindowId(movedPinnedWindowId);
            var currentPinnedTabsUrl = [];

            currentSession.tabs.forEach(function(tab, index){
                currentPinnedTabsUrl.push(tab.url);
            });

            this.sessions.forEach(function (session) {
                if(session.windowId != movedPinnedWindowId ){
                    var pinned_changed = false;
                    session.tabs.forEach(function(tab, index){
                        var currentTabIndex = currentPinnedTabsUrl.indexOf(tab.url);
                        if( currentTabIndex>=0 && tab.pinned == true ){
                            tab.index = currentSession.tabs[currentTabIndex].index;
                            pinned_changed = true;                         
                        }  
                    });
        ...
        // when remove or uppin pinned tab, remove the tab from another seesions.
        handlePinnedTabUpdated: function(updatedPinnedTab){

            this.sessions.forEach(function (session) {
                if(session.windowId != updatedPinnedTab.windowId ){
                    var pinned_changed = false;
                    session.tabs.forEach(function(tab, index){
                        if( tab.url == updatedPinnedTab.url && tab.pinned == true ){
        ...
        updateSpaceOrder: function(orders, callback) {
            ...
            orders.forEach(function(order){
                session = self.getSessionBySessionId( parseInt(order[0], 10) );
        ...
        updateSpacePosition: function(position, windowId, callback) {
            var self = this, 
                session = session = this.getSessionByWindowId(windowId);
